...
GraphQL 规范定义了方便人们阅读的 **schema definition language**(**SDL**)，SDL 用来定义 schema 的结构并且以字符串的形式存储。
...
因为这些关系被定义在规范的 schema 中，客户端开发人员能够知道哪些数据是可用的，并且在一个精简的 query 中查询需要的数据。

注意，schema **不定义**数据的来源与存储方式。schema 完全与实现无关。
